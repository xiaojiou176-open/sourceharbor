...
| Official MCP Registry listing + metadata template | **ship-now** | the repo ships a registry-ready metadata template and package line at `0.1.19`, but a fresh anonymous registry index read-back does not currently return `SourceHarbor`, so live listing truth is still unverified until publish/refresh plus read-back exists |
...
- official-surface public distribution is now split more sharply:
  - PyPI is still publicly live at `0.1.14`, but Official MCP Registry live truth is currently unverified in a fresh anonymous read-back; both public package/distribution surfaces still lag or remain gated versus the repo-controlled `0.1.19` package line
...
| Official MCP Registry + PyPI version refresh | fresh PyPI JSON read-back still shows the public `sourceharbor` package at `0.1.14`, while a fresh anonymous Official MCP Registry index read-back does not currently return `SourceHarbor`; the repo-controlled package and directory packet remain at `0.1.19` | the repo now has a successful `uv build` receipt for the current package line; the remaining blockers are publish credentials plus a fresh registry read-back that proves the live entry | once the publish credentials exist, publish the intended current package version, refresh the registry path if needed, and read both surfaces back before repeating a live claim |
