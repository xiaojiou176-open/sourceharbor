...
- **Vendor radar starter lane:** `/subscriptions` now exposes an official vendor-channel desk for OpenAI, Anthropic, Gemini, and xAI; the catalog distinguishes confirmed changelog/status/blog truth from X fast signals, and `/watchlists` can now start a vendor watchlist from those templates without inventing a second intake front door
...
- **Bilibili hardening lane:** the worker now carries adaptive Bilibili ASR model selection driven by duration/language/quality heuristics, the live-smoke path can read a curated 5-10 sample canary matrix, and diagnostics now keep one explicit manual-intake -> consume -> reader/public-boundary receipt instead of relying on a single static Bilibili URL
- **Bilibili evidence + ops surfaces:** job bundles now surface richer Bilibili evidence including danmaku, creator/video metadata, and commentary summaries; `/jobs` and the feed reading pane can surface those source facts, while `/ops` now exposes repo-owned browser proof plus the current Bilibili account read-only ops gate
- **Reader pipeline:** `ClusterVerdictManifest`, `PublishedReaderDocument`, `CoverageLedger`, `TraceabilityPack`, `repair` modes, and `NavigationBrief` now exist as repo-side runtime surfaces over `ConsumptionBatch`, with `published_with_gap` documents kept available for repair/internal audit while public reader routes now surface only fully published documents
...
| Bilibili account center | local account-proof anchor for the strong-supported Bilibili source lane | URL / page-state proof today, hybrid later if stronger account-side automation is justified | human login in the repo-owned Chrome profile; cookie-driven richer read-only lanes stay repo-local only | **external-blocked** |
...
- official-surface public distribution is now split more sharply:
  - exact package and directory read-back state now stays centralized in [public-distribution.md](./public-distribution.md), so this page only keeps the blocker families: publish credentials/auth, third-party maintainer review, and manual submit/read-back steps
