...
- **MCP**: the root Python package now produces the real `sourceharbor-mcp` install artifact, the registry template points at the PyPI identifier `sourceharbor`, and the PyPI project still has a live public read-back at `0.1.14`; a fresh anonymous Official MCP Registry index read-back does not currently return `SourceHarbor`, so the registry line should be treated as unverified until publish/refresh plus read-back exists. The repo-controlled package line is `0.1.19`.
