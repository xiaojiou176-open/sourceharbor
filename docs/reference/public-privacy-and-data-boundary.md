...
- vendor-radar starter packets under `/subscriptions` may safely expose public vendor URLs, signal-layer labels, and starter watchlist defaults, but they must never turn personalized watchlist state, maintainer browsing history, or signed-in vendor account context into public repo data
