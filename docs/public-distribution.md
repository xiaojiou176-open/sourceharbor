...
| **Official MCP Registry** | root `pyproject.toml` + `sourceharbor-mcp` console script + `starter-packs/mcp-registry/sourceharbor-server.template.json` | **fresh anonymous registry read-back confirms a live active entry** for `io.github.xiaojiou176-open/sourceharbor-mcp` at public version `0.1.14`, while the repo-tracked package and directory packet sit at `0.1.19`; the current line is build-ready, but refresh still depends on publish credentials and registry auth | publish/refresh the intended package version when credentials are intentionally available, then capture the refreshed registry listing/API read-back before repeating a newer snapshot claim; official automation paths exist via GitHub Actions OIDC or token-backed auth exchange | registry listing URL/API read-back, published package version, or namespace/publish blocker |
