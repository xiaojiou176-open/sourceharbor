...
| **Official MCP Registry** | root `pyproject.toml` + `sourceharbor-mcp` console script + `starter-packs/mcp-registry/sourceharbor-server.template.json` | **fresh anonymous registry index read-back does not currently return a public `SourceHarbor` entry**, while the repo-tracked package and directory packet sit at `0.1.19`; the current line is build-ready, but live registry truth should be treated as unverified until publish/refresh plus read-back exists | publish/refresh the intended package version when credentials are intentionally available, then capture a registry listing URL or API read-back before repeating a live-entry claim | registry listing URL/API read-back, published package version, or namespace/publish blocker |
| **PyPI (`sourceharbor`)** | root `pyproject.toml` + `sourceharbor-mcp` console script | **fresh JSON read-back confirms a live PyPI project** at version `0.1.14`; the repo-controlled package version is still `0.1.19`, and the current line is build-ready but refresh is blocked by missing publish credentials | publish the newer package version only when release/package credentials are intentionally available, then read the version back from PyPI | package URL/API read-back, published package version, or publish blocker |
| **MCP.so** | `config/public/mcp-directory-profile.json` + `docs/media-kit.md` + `docs/assets/sourceharbor-square-icon.png` | the repo-side listing inputs are ready, but a fresh anonymous direct read-back at `/server/sourceharbor-mcp` currently returns **`403 Forbidden`**; there is still no public live listing proof on that route today | retry only when you can keep the flow auditable end-to-end, then capture either a real receipt or the exact receiptless stop point | submission receipt, pending-review URL, live listing URL, or the exact `403` / sign-in blocker |
| **PulseMCP** | `config/public/mcp-directory-profile.json` + `docs/media-kit.md` + `docs/assets/sourceharbor-square-icon.png` | anonymous public read-back currently stops at **`Access Denied`**; search-engine snippets still suggest a SourceHarbor MCP surface, but direct public listing truth remains platform-controlled until a readable page or receipt exists | use the registry-linked or editor-managed route only when you want a stronger listing, and capture the exact path that answered | listing URL, acknowledgment email, registry-linked listing, or exact platform blocker |
