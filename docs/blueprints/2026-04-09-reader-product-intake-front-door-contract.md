...
| 输入形态 | 当前状态 | 为什么不能假装支持 |
| --- | --- | --- |
| 通用 article detail URL | `manual_injected SourceItem` | 当前会先进入 today lane，再和现有 tracked universe 做 best-effort 回认 |
| “靠 URL 自动还原所有 creator identity” 的任意网站页 | `unsupported` | 当前只有 YouTube/Bilibili/RSSHub/feed 这几类前门规则 |
| 未知 host 的 raw URL | `manual_injected SourceItem` | 走 generic article one-off lane；若后端安全校验失败再显式拒绝 |
...
| manual one-off article item | one-off article bridge 已接入 | 进入 today lane，并与 `manual_injected SourceItem -> ConsumptionBatch` 主链对齐 |
